function pass = GARP(q,p,fosd)
% GARP       : Check GARP condition
...
for i = 1:T
    if fosd == 0
        ...
        P0(i,:) = (p(i,:)*q(i,:)'>p(i,:)*q')';
    elseif fosd == 1
        R0(i,:) = ((p(i,:)*q(i,:)'>=p(i,:)*q')' | (p(i,:)*q(i,:)'>=p(i,:)*qM')');
        P0(i,:) = ((p(i,:)*q(i,:)'>p(i,:)*q')' | (p(i,:)*q(i,:)'>p(i,:)*qM')');
    end
    
    % Correct for diagonal element of P0 (which cannot be 1)
    P0(i,i) = 0;
end
