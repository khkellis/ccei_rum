...
%   
%   WARNING:
%   Do not set (budget_length,number_classes) > {(8,3),(7,4),(6,5)}
%   otherwise program takes too long to run (time increases exponentially).
%
...
%   calculate test statistic.  Time increases linearly with bs_reps.
%   With  (budget_length,number_classes) = (8,3) or (7,4) and bs_reps=1000,
%   the program takes about 3-12 hours depending on the number of cores.  
%
%   Other parameters:
...
cores            = 32;      % Number of cores for parallel computing 
...
% Number of bootstrap repetitions.  bootstrap_reps = 1000 takes about 3-12 
% hours to complete depending on classes and budget_l.
...
if flag_estimator == 0
    % Compute Jstat and bootstrap distribution using Kernel Smoothing
    for ii = 1:budget_n
        [pi_hat{ii},Jstat(ii,1),Jstat_bs{ii},CV(ii,:), pval(ii,1)] ...
            =  RUM_61_KernelStatistics(budgets{ii},cell(shares_all(ii:ii+(budget_l-1))),cell(income_all(ii:ii+(budget_l-1))),X{ii},A{ii});
...
elseif flag_estimator == 1
    ...
    for ii = 1:budget_n
        [pi_hat{ii},Jstat(ii,1),Jstat_bs{ii},CV(ii,:), pval(ii,1),tau(ii,1),eta_hat{ii}] ...
            =  RUM_51_BasisStatistics(budgets{ii},cell(shares_all(ii:ii+(budget_l-1))),cell(income_all(ii:ii+(budget_l-1))),X{ii},A{ii});
        % Input of above function is budgets in period ii, the structure of
        % shares which is [num_household,classes]{budget_l} and income, which
        % is [num_household,1]{budget_l}.  Also input A and X for budget-period
        % ii.
...
elseif flag_estimator == 2
    ...
    for ii = 1:budget_n
        [pi_hat{ii},Jstat(ii,1),Jstat_bs{ii},CV(ii,:), pval(ii,1), tau(ii,1),eta_hat{ii}] ...
            =  RUM_71_EndogenousStatistic(budgets{ii},cell(shares_all(ii:ii+(budget_l-1))),cell(income_all(ii:ii+(budget_l-1))),cell(instrument_all(ii:ii+(budget_l-1))),X{ii},A{ii});
        % Input of above function is budgets in period ii, the structure of
        % shares which is [num_household,classes]{budget_l} and income, which
        % is [num_household,1]{budget_l}.  Also input A and X for budget-period
        % ii.
